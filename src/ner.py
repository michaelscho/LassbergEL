...
def _warn_transformers_for_hmbert() -> None:
    ...
    try:
        ...
        print(f"Unknown transformers version string: {ver}")
        return
    if not ((major == 4 and 30 <= minor <= 43) or (major == 4 and minor == 36)):
        print(f"transformers {ver} may not workwith hmBERT remote pipeline.")
...
def _run_hmbert_generic_ner(text: str, model_id: str, device: str = "cpu",
    ...
    try:
        ...
        print(f"Could not load '{model_id}' ({e}). Falling back to '{fallback}'.")
...
# flair-large external environment
...
def _run_flair_large_external(text_file: Path, out_json: Path,
    ...
    if proc.returncode != 0:
        print("External Flair-large worker failed.")
