...
@dataclass
class DocumentInfo:
    """representation of a server-side document."""
    id: int
    title: str
...
def _coerce_selection_to_docs(
    ...
    for item in items:
        if isinstance(item, DocumentInfo):
            ...
            # If InquirerPy returns dict-like structures
            try:
...
# InquirerPy interactive selector
...
def select_documents_interactive(
    ...
    *,
    ...
    if backend == "transkribus":
        ...
        client = EScriptoriumClient(**(transkribus_kwargs or {}))  # not implemented yet
    else:
